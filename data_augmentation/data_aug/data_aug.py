...
class RandomTranslate(object):
    ...
    def __call__(self, img, bboxes):        
        ...
        bboxes_ = bboxes
        if y >0 :
            img_y = img_[y:img_.shape[0],:,:]
            bboxes_[:, :4] -= [0, y, 0, y]
        else:
            img_y = img_[0:img_.shape[0] + y, :, :]
        if x >0 :
            img_y = img_y[:,x:img_.shape[1],:]
            bboxes_[:, :4] -= [x, 0, x, 0]
        else:
            img_y = img_y[:, 0:img_.shape[1] + x, :]
        bboxes_ = clip_box(bboxes_, [0,0,img_shape[1], img_shape[0]], 0.25)
        return img_y, bboxes_
